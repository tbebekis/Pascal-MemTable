...
(*----------------------------------------------------------------------------*)
  ...
  TFilterParser = class(TCustomExpressionParser)
  private
    FDataset: TDataSet;
  ...
  public
    ...
    function FilterRecord(RecBuf: TRecordBuffer): Boolean;
...
constructor TFilterParser.Create(ADataset: TDataset);
begin
  FDataset := Adataset;
...
function TFilterParser.GetResultType: TExpressionType;
begin
...
procedure TFilterParser.SetCaseInsensitive(NewInsensitive: Boolean);
begin
...
procedure TFilterParser.SetPartialMatch(NewPartialMatch: boolean);
begin
...
procedure TFilterParser.FillExpressList;
var
  lExpression: string;
begin
...
function TFilterParser.GetVariableInfo(VarName: string): TField;
begin
  Result := FDataset.FindField(VarName);
end;

function TFilterParser.CurrentExpression: string;
begin
  Result := FCurrentExpression;
end;

procedure TFilterParser.HandleUnknownVariable(VarName: string);
var
  FieldInfo: TField;
...
procedure TFilterParser.ClearExpressions;
var
  I: Integer;
begin
  inherited;
...
procedure TFilterParser.ParseExpression(AExpression: string);
var
...
function TFilterParser.ExtractFromBuffer(Buffer: TRecordBuffer): PChar;
var
  I: Integer;
begin
...
function TFilterParser.FilterRecord(RecBuf: TRecordBuffer): Boolean;
begin
  Result := Boolean((ExtractFromBuffer(RecBuf))^);
end;
